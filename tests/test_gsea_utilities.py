...
def test_get_rank_files_reads_from_DE_zip():
    ...
    for key, df in dataframes.items():
        ...
        assert not df.is_empty(), f"DataFrame for {key[1]}.rnk in {DE_ZIP_FILENAME} is empty."
        # Assuming standard rank file format (Identifier, Rank_Value)
        assert df.shape[1] == 2, f"Expected 2 columns in {key[1]}.rnk from {DE_ZIP_FILENAME}, found {df.shape[1]}"
