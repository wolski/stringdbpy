...
class TermNetworkPlotter:
    ...
    def _prepare_graph(
        self, edge_df: pl.DataFrame, thresh: int = 2, include_all: bool = False
        ...
        if include_all:
            ...
            nodes = (
                edges_sub.select(["termID_a", "termID_b"])
    ...
    def compute_full_layout(
        self, edge_df: pl.DataFrame, thresh: int = 2, k: float = 0.8, seed: int = 0
...
def plot_network(
    ...
    if contrast is None:
        ...
        plotter.draw_panel(
            ...
            all_df,
            thresh=thresh,
            title=f"Network for {contrasts[0]}, threshold = {thresh}",
            use_fixed_layout=use_fixed_layout,
        )
