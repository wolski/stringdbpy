...
def make_network(xdf: pl.DataFrame) -> nx.Graph:
    ...
    proteins_pl = (
        edges_pl.select(["proteinLabels", "proteinInputValues"])
        .unique()
        .with_columns(
            [
    ...
    edge_list_pl = edges_pl.rename({"termID": "from", "proteinLabels": "to"}).select(
        ["from", "to"]
    )
...
def assign_node_colors(G, n_colors: int = 100):
    ...
    cmap = LinearSegmentedColormap.from_list(
        "diverge", ["green", "gray", "red"], N=n_colors
    )
    half = n_colors // 2
    ...
    direction_color_map = {"top": "yellow", "bottom": "blue", "both_ends": "green"}

    # 3) Term nodes: compute fill & border
    term_nodes = [n for n, d in G.nodes(data=True) if d["nodeType"] == "term"]
    term_vals = np.array(
        [G.nodes[n]["meanInputValues"] for n in term_nodes], dtype=float
    )
    ...
    prot_vals = np.array(
        [G.nodes[n]["proteinInputValues"] for n in prot_nodes], dtype=float
    )
...
def build_tooltip(node_id: str, data: dict) -> str:
    ...
    if data["nodeType"] == "term":
        ...
        return f"<b>Protein:</b> {node_id}<br/><b>Value:</b> {prot_val:.3f}"
    ...
    cw.set_style(
        [
            ...
            # 2) Protein nodes (unchanged)
            {
                ...
                "style": {"content": "", "background-opacity": 0.5},
            },
            ...
            {
                "selector": "edge",
                "style": {"line-color": "#cccccc", "line-opacity": 0.5, "width": 1},
            },
        ]
    )
...
def bipartite_hybrid_layout(
    ...
    sorted_prots = sorted(prots_init, key=lambda p: prot_bary[p], reverse=True)
...
def plot_network_graph_plotly(G, title: str, layout: str = "bipartite"):
    ...
    # use plain <br> here
    prot_hover = [
        build_tooltip(n, G.nodes[n]).replace("<br/>", "<br>") for n in prot_nodes
    ]
    ...
    term_edge_colors = [G.nodes[n].get("borderColor", "gray") for n in term_nodes]
    term_hover = [
        build_tooltip(n, G.nodes[n]).replace("<br/>", "<br>") for n in term_nodes
    ]
